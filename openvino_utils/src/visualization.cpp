...
void Visualization::mergeDetectionMsgs()
{
  ...
  //std::unique_ptr<CamSub> cam_sub = std::make_unique<CamSub>(this, "/rdk/openvino/image_raw");
  std::unique_ptr<CamSub> cam_sub = std::make_unique<CamSub>(this, "/camera/color/image_raw");
  //std::unique_ptr<ObjSub> obj_sub = std::make_unique<ObjSub>(this, "/rdk/openvino/detected_objects");
